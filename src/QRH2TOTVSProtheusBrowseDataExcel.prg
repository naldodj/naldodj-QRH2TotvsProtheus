...
*/
Function QRH2TOTVSProtheusBrowseDataExcel(oRecordSet,cTitle,cFilterField)
   ...
   DEFINE WINDOW Form_10 At 40,60 ;
    ...
    DEFINE SPLITBOX
      DEFINE TOOLBAREX Bar_1 BUTTONSIZE 28, 28
         ...
         BUTTON Btn_2 PICTURE "QRH2TOTVSProtheusExcel16" ;
            ACTION fExcel( oBrw ) ;
            TOOLTIP "Export Browse to Excel"

         BUTTON Btn_3 PICTURE "QRH2TOTVSProtheusExitb16" ;
      ...
      BUTTON Btn_1a PICTURE "Tick" ;
            ACTION {|| ChangeFiltr(oBrw,cFilterField,oRecordSet)};
            CHECK ;
      ...
      COMBOBOX Combo_1 ;
         ...
         FONT 'Tahoma' SIZE 9 ;
         TOOLTIP 'List of '+cFilterField;
         ON CHANGE ChangeFiltr(oBrw,cFilterField,oRecordSet)
    ...
    END SPLITBOX
      ...
      @ 35, 0 TBROWSE oBrw RECORDSET oRecordSet  WIDTH 800 HEIGHT 480  ;
           FONT "Font_1" CELL
      oBrw:LoadFields()
      oBrw:Exchange(3, 5)
      /*
...
FUNCTION ChangeFiltr(oBrw,cFilterField,oRecordSet)
   LOCAL pos , cSelFilterField

   IF Form_10.Btn_1a.Value == .t.
      pos:=Form_10.Combo_1.Value
      IF pos >0
          cSelFilterField := SubStr(M->aFilterField[pos],1,2)
          oBrw:SetFilter( cFilterField, cSelFilterField )
          oBrw:cPrefix := cSelFilterField
          with object oRecordSet  
            IF (:Fields(cFilterField):Value != cSelFilterField)
                oBrw:Enabled(.f.)
            ELSE
                oBrw:Enabled(.T.)
            endif
   ...
   ELSE
      cSelFilterField := ""
      oBrw:SetFilter( "", cSelFilterField )
      oBrw:Enabled(.T.)
      oBrw:Refresh( .T. )
   ENDIF
...
FUNCTION freport(oBrw)
   ...
   oBrw:Report( "Report", {1,2,3,4}, .T., .F., .T.,.F.  )
...
Function fExcel( oBrw, cFile, cTitle )
   ...
   Default cFile  := Padr( "NoName.xlsx", 60 ), ;
...
RETURN PutFile({{"Excel Book (*.xlsx)","*.xlsx"}},"Select the file",,.T.,cFile)
